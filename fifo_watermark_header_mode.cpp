...
#define SENSOR_TIME_MAX 0xFFFFFF  // 24-bit max: 16,777,215
...
int main(int argc, char** argv)
{
    ...
    auto ros_node = rclcpp::Node::make_shared("bmi270");
    auto imu_pub = ros_node->create_publisher<sensor_msgs::msg::Imu>("imu", rclcpp::QoS(200).best_effort().durability_volatile());
    ...
    if (rslt == BMI2_OK) {
        int64_t sensortime = 0;
        sensortime = (int64_t)sensortime_raw[0] | ((int64_t)sensortime_raw[1] << 8) | ((int64_t)sensortime_raw[2] << 16);
        //printf("sensor time (s) %f\n", sensortime * BMI2_SENSORTIME_RESOLUTION);
        int64_t sensortime_ns = (sensortime * 390625) / 10;
        printf("local_ns / sensortime_ns: %ld / %ld\n", local_ns, sensortime_ns);
        time_offset_ns = local_ns - sensortime_ns;
    }

    uint32_t last_sensortime = 0;
    ...
    while (rclcpp::ok())
    {
        ...
        if ((rslt == BMI2_OK) && (int_status & BMI2_FWM_INT_STATUS_MASK))
        {
            ...
            if (rslt == BMI2_OK)
            {
                ...
                {
                    printf("%d, %f, %f, %f\n", index, fifo_accel_data[index].x * acc_scale, fifo_accel_data[index].y * acc_scale, fifo_accel_data[index].z * acc_scale);
                }*/
                ...
                {
                    printf("%d, %f, %f, %f\n", index, fifo_gyro_data[index].x * gyro_scale, fifo_gyro_data[index].y * gyro_scale, fifo_gyro_data[index].z * gyro_scale);
                }*/

                uint32_t sensortime = fifoframe.sensor_time;
                if (sensortime < last_sensortime) { // rollover
                    sensortime = SENSOR_TIME_MAX - last_sensortime + sensortime + 1;
                }
                local_ns = ((int64_t)sensortime * 390625) / 10 + time_offset_ns - 5000000 * ( accel_frame_length - 1);
                for (index = 0; index < accel_frame_length ; index++) {
                    sensor_msgs::msg::Imu imu_msg;
                    imu_msg.header.frame_id = "body";
                    imu_msg.header.stamp.sec = static_cast<int32_t>(local_ns / 1000000000);
                    imu_msg.header.stamp.nanosec = static_cast<uint32_t>(local_ns % 1000000000);
                    imu_msg.linear_acceleration.x = fifo_accel_data[index].x * acc_scale;
                    imu_msg.linear_acceleration.y = fifo_accel_data[index].y * acc_scale;
                    imu_msg.linear_acceleration.z = fifo_accel_data[index].z * acc_scale;
                    imu_msg.angular_velocity.x = fifo_gyro_data[index].x * gyro_scale;
                    imu_msg.angular_velocity.y = fifo_gyro_data[index].y * gyro_scale;
                    imu_msg.angular_velocity.z = fifo_gyro_data[index].z * gyro_scale;
                    imu_pub->publish(imu_msg);
                    local_ns += 5000000;
                }
                last_sensortime = fifoframe.sensor_time;
